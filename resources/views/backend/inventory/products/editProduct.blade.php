    ...
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                @endif
                <form method="POST" action="{{route('backend.inventory.products.update', $product->product_id)}}" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="card">
                        @if ($errors->any())
    ...
    <!-- Modals -->
        <!-- Add-stock -->
            <div class="modal fade" id="add-stocks" aria-hidden="true" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-default modal-center">
                    <form method="POST" action="{{route('backend.inventory.products.addStocks')}}">
                        @csrf
                        @method('PATCH')
        ...
        <!-- Remove-Defectives -->
            <div class="modal fade" id="remove-defectives" role="dialog" aria-hidden="true" tabindex="-1">
                <div class="modal-dialog modal-default modal-center">
                    <div class="modal-content">
                        <form method="POST" action="{{route('backend.inventory.products.removeDefectives')}}">
                            @csrf
                            @method('PATCH')
                            <div class="modal-header">
        ...
        <!-- Password Confirmation modal -->
            <div class="modal fade" id="password-confirm" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-default modal-center">
                    <form method="POST" action="{{route('backend.inventory.products.destroy',$product->product_id)}}">
                    @csrf
                    @method('DELETE')
                        <div class="modal-content">
                            <div class="modal-header">
